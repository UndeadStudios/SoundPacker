...
public class Class16 {
    ...
    public static final void method240(int i, OutputStream stream2) throws IOException {
        aByteArray327 = new byte[0x6baa8];
        Stream stream = new Stream(aByteArray327, 891);
        ...
        Class6.method166();
        do {
            int j = aStream_328.readUnsignedShort();
            if (j == 65535) {
                return;
            }
            aClass16Array325[j] = new Class16(8);
            aClass16Array325[j].method242(true, stream, stream2);
    ...
    private final void method242(boolean flag, Stream stream, OutputStream stream2) throws IOException {
        for (int i = 0; i < 10; i++) {
            int j = stream.readUnsignedShort();
            if (j != 0) {
                stream.currentPosition--;
                aClass6Array329[i] = new Class6();
                aClass6Array329[i].method169(new DataOutputStream(stream2));
            }
        }
